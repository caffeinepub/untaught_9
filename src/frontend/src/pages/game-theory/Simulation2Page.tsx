...
import { Sim1BPriceWar } from "./sims/Sim1BPriceWar";
import { Sim2BArmsRace } from "./sims/Sim2BArmsRace";
import { Sim3BPricingGame } from "./sims/Sim3BPricingGame";
import { Sim4BSupplierTrust } from "./sims/Sim4BSupplierTrust";
...
export function Simulation2Page({ moduleId }: Simulation2PageProps) {
  ...
  const handleComplete = () => {
    markDone(module.simulation2Id ?? "", {
      onSuccess: () => {
        void navigate({ to: `/domain/game-theory/module/${moduleId}/quiz` });
      },
      onError: () => {
        void navigate({ to: `/domain/game-theory/module/${moduleId}/quiz` });
      },
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12 accent-indigo">
      ...
      >
        Apply what you've learned — this simulation presents a real-world
        scenario. Explore the consequences of each strategy.
      </div>
