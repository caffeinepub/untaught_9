...
const QUIZZES = [M1_QUIZ, M2_QUIZ, M3_QUIZ, M4_QUIZ, M5_QUIZ];
const PASS_THRESHOLDS = [2, 2, 2, 2, 2];
...
export function QuizPage({ moduleId }: QuizPageProps) {
  ...
  const handleSubmit = () => {
    ...
    submitQuiz(
      { quizId: module.quizId, score, passed },
      {
        onSuccess: () => {
          if (passed) {
            markModuleComplete(`behavioural-economics-module-${moduleId}`);
          }
        },
      },
    );
  ...
  const handleNext = () => {
    void navigate({
      to: `/domain/behavioural-economics/module/${moduleId}/wild`,
    });
  };
    ...
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12 accent-amber">
      ...
      <div className="space-y-6 mb-8">
        {questions.map((q, qi) => {
          ...
          return (
            <div
              key={q.id}
              data-ocid={`be_quiz.question.item.${qi + 1}`}
      ...
      {!results ? (
        <Button
          ...
          data-ocid="be_quiz.submit.button"
